...
\newpage
Donde la metodología CAMEL tiene por objeto evaluar la **solidez financiera** de las instituciones con base ha indicadores cuantitativos, contemplando cinco características: 
...
A continuación, se definen los indicadores que componen a la metodología CAMEL, en función de los elementos que la componen definidos por la @MANUAL_CUENTAS_ASFI en su manual de cuentas y la formulas expuestas por @RATIOS-CAMEL:
...
Este indicador definido en la ecuación \@ref(eq:mt-cccm) mide o tiene objeto responder si el patrimonio de la institución cubre en tanto por ciento:
...
El indicador definido en la ecuación \@ref(eq:mt-caccm) mide o tiene objeto responder si el patrimonio de la institución cubre en tanto por ciento:
...
Este indicador definido en la ecuación \@ref(eq:mt-ccp) mide o tiene por objeto responder si los activos descontando las cuentas contingentes cubren los el patrimonio de la misma.
...
En esta sección de la metodología CAMEL se establecen rangos a los cuales le corresponde una calificación, sujeta a una probabilidad [@TENDENCIAS-CAMEL], es decir, aquellos resultados mejores, pero menos probable se los posiciona en L1 dando una mejor calificación y aquellos resultados peores y menos probables se los posiciona en L5 recibiendo una peor calificación, es decir, los límites de los indicadores están definidos por la desviación estándar y el promedio de los datos históricos:
...
El termino de pronóstico definido como "la acción y efecto de pronosticar" [@RAE], así mismo pronosticar es definido como "predecir algo en el futuro a partir de indicios" [@RAE], es decir, el pronóstico es el proceso de estimación en situaciones de incertidumbre, para los propósitos de esta investigación, un pronóstico es un evento asociado a una distribución de probabilidad.
...
```{r mt-funciones-de-activacion, fig.cap='Funciones de activación', out.height='40%', out.width='100%'}
plotActFuntMarcTeor()
```

ver la figura \@ref(fig:mt-funciones-de-activacion).
