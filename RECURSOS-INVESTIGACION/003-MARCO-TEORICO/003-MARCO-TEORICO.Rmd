...
```{r mt-funciones-de-activacion, fig.cap='Funciones de activación', fig.height = 30, fig.width = 20}
plotActFuntMarcTeor()
```
...
- La primera capa oculta procesará los datos de la capa de entrada toma la siguiente forma:
...
Ahora bien, los modelos alternativos empleados en contraposición de las redes neuronales para realizar las proyecciones son los siguientes:
...
Así mismo, las especificaciones del algoritmo utilizado para el presente modelo corresponden al paquete "forecast" del repositorio publico CRAN [@R-forecast].
...
Entendiendo la tendencia como la tasa de cambio de la serie de tiempo respecto al tiempo y las estacionalidades como tasas de cambio correspondientes a variables dicótomas, es decir, solo pueden asumir valores de 0 o 1 cuando la observación se encuentre en la estacionalidad dada, así mismo el modelo presentado asume una estacionalidad de 12 periodos que en consecuencia la ecuación \@ref(eq:drnn-st) toma la forma de la ecuación \@ref(eq:drnn-stY).
