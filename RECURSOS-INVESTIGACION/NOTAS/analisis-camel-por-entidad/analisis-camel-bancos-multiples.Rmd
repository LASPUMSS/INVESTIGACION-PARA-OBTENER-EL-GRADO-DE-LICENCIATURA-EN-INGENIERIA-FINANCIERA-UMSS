...
## Filtrar resultados por entidad
entidad <- c('BCR','BDB','BEC','BFO','BFS','BGA','BIE','BIS','BME','BNA','BNB','BPR','BSO','BUN')

datCamelCalificacionIndicadores <- datCamelCalificacionIndicadores %>%
    filter(TIPO_DE_ENTIDAD %in% entidad )
datCamelCalificacionIndicadoresHard <- datCamelCalificacionIndicadoresHard %>%
    filter(TIPO_DE_ENTIDAD %in% entidad )

datCamelCalificacionEntidadSinPoderar <- datCamelCalificacionEntidadSinPoderar %>%
    filter(TIPO_DE_ENTIDAD %in% entidad )
datCamelCalificacionEntidadSinPoderarHard <- datCamelCalificacionEntidadSinPoderarHard %>%
    filter(TIPO_DE_ENTIDAD %in% entidad )

datCamelCalificacionEntidad <- datCamelCalificacionEntidad %>% 
    filter(TIPO_DE_ENTIDAD %in% entidad )
datCamelCalificacionEntidadHard <- datCamelCalificacionEntidadHard %>%
    filter(TIPO_DE_ENTIDAD %in% entidad )
...
## Resultados CAMEL de bancos multiples
...
listTablesRender <- renderListTablesInd(listResult, 'Tendencia y estadisticas C-A-M-E-L con rangos flexibles sin ponderar', 14)
```
...
listTablesRender <- renderListTablesInd(listResult, 'Tendencia y estadisticas C-A-M-E-L con rangos estrictos sin ponderar', 14)
```
...
listPlots <- getListPlotsInd(listResult)
listTablesRender <- renderListTablesInd(listResult, 'Tendencia y estadisticas CAMEL', 14)
```
