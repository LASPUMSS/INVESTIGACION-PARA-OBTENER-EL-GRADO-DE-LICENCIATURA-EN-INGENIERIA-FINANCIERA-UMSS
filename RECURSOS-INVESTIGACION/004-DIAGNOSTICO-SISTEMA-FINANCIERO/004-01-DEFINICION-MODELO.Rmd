...
El numero de observaciones de la datos manejados por esta investigación alcanzan a `r nrow(dat)`, donde cada observación corresponde a un balance general y un estado de resultados en un punto del tiempo entre las fechas (`r min(dat$FECHA)` - `r max(dat$FECHA)`), dichos estados financieros se clasifican o agrupan por sectores financieros los cuales son:
...
Las variables comprendidas en el modelo del diagnostico están sujetas al manual de cuentas para entidades financieras, definidas por la autoridad de supervisión del sistema financiero.
...
Las inversiones temporarias (designado por el código 120.00 en el manual de cuentas) según [@MANUAL_CUENTAS_ASFI] registran:
...
Los pasivos (designado por el código 200.00 en el manual de cuentas) según [@MANUAL_CUENTAS_ASFI] se agrupan las cuentas representativas de las obligaciones directas de la entidad la que comprenden:
...
La variable impuestos esta compuesta por dos elementos del manual de cuentas:

- Impuestos
- Impuesto a las utilidades de las empresas (IUE)

Donde según [@MANUAL_CUENTAS_ASFI] los impuestos (designado por el código 455.00 en el manual de cuentas) registran los gastos devengados en el período por concepto de impuestos, asimismo los intereses y recargos originados por los mismos. El gasto por Impuesto al valor agregado que se registra en esta cuenta, es el que surge de aplicar la tasa del impuesto sobre ingresos presuntos, tales como el que proviene de tomar en anticrético un bien. 

Así también el impuesto a las utilidades de las empresas (designado por el código 460.00 en el manual de cuentas) comprende los gastos devengados en el período por concepto de impuesto sobre las utilidades de las empresas.

#### Resultado operativo bruto (ROB)

Esta variable de resultado operativo bruto esta definida por la siguiente ecuación:

$$ R.O.B. = \text{Ingresos Financieros} - \text{Gastos Financieros} \pm \text{Otros (Ingresos/Gastos) Operativos}   $$



#### Resultado neto de la gestión (RNG)

Esta variable de resultado neto de la gestión esta definida por la siguiente ecuación:

$$ R.N.G. = \text{Resultado Operativo Bruto} \pm \text{Otros (Ingresos/Gastos) Extraordinarios} - \text{IUE} $$
